...
U8_T GUDPMC_NewConn(U32_T XDATA* pip, U16_T remotePort, U8_T socket)
{
	...
	printd("udpmc new connection, socket=%bu, remoteport=%u, ip=%lu\r\n", socket, remotePort, *pip);
	for (i = 0; i < GUDPMC_MAX_CONNS; i++)
	{
...
{
    printd("udpmc event, id=%bu, event=%bu\r\n", id, event);
...
void GUDPMC_Receive(U8_T XDATA* pData, U16_T length, U8_T id)
{
	...
	printd("udpmc recevie, id=%bu, length=%u\r\n", id, length);
...
void gudpmc_HandleUpgradeReq(U8_T XDATA* pData, U16_T length, U8_T id)
{	
	printd("gudpmc_HandleUpgradeReq()\r\n");

	GCONFIG_EnableFirmwareUpgrade(1);
