...
static U32_T gtcpdat_LastConnectTime; //zwh.add 20120302
        ...
        switch (gtcpdat_Conns[i].State)
        {
            case GTCPDAT_STATE_FREE:
                if (gtcpdat_TcpClient == 1)
                {
					...
					if ((SWTIMER_Tick()-gtcpdat_LastConnectTime) < 20)		 //zwh.add 20120302
					    break;												 //zwh.add 20120302
...
static void gtcpdat_GetDataFromUr(U8_T id)
{
    ...
    if (availCount > 16)
	{
		printd("uart recv [%u]: %bx %bx %bx %bx %bx %bx %bx %bx ... %bx %bx %bx %bx %bx %bx %bx %bx\r\n"
			availCount, pUpBuf[0], pUpBuf[1], pUpBuf[2], pUpBuf[3], pUpBuf[4], pUpBuf[5], pUpBuf[6], pUpBuf[7],
		   				pUpBuf[availCount-8], pUpBuf[availCount-7], pUpBuf[availCount-6], pUpBuf[availCount-5], 
						pUpBuf[availCount-4], pUpBuf[availCount-3], pUpBuf[availCount-2], pUpBuf[availCount-1]);
...
void GTCPDAT_Init(U16_T localPort)
{
	...
	gtcpdat_LastConnectTime = 0; //zwh.add 20120302
...
U8_T GTCPDAT_NewConn(U32_T XDATA* pip, U16_T remotePort, U8_T socket)
{
	...
	COUNTER_INC_CONNECT_TRY;	   //zwh.add 20120302
	for (i = 0; i < GTCPDAT_MAX_CONNS; i++)
	{
	...
	COUNTER_INC_CONNECT_FAILED;	//zwh.add 20120302
...
void GTCPDAT_Event(U8_T id, U8_T event)
{
	...
	if (event == TCPIP_CONNECT_ACTIVE)
	{
		if (gtcpdat_Conns[id].State == GTCPDAT_STATE_WAIT)
		{
			...
			COUNTER_INC_CONNECT_SUCCESS;	//zwh.add 20120302
		}
	}
	else if (event == TCPIP_CONNECT_CANCEL)
	{
		if (gtcpdat_Conns[id].State == GTCPDAT_STATE_CONNECTED)
		{
			...
			gtcpdat_Conns[id].Timer = SWTIMER_Tick();	//zwh.add 20120302
			COUNTER_INC_CONNECT_CLOSED;					//zwh.add 20120302
		}
		else if (gtcpdat_Conns[id].State == GTCPDAT_STATE_WAIT)
		{
			gtcpdat_Conns[id].State = GTCPDAT_STATE_FREE;
			GS2W_SetTaskState(GS2W_STATE_IDLE);	
