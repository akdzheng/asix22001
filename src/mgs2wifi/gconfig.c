...
void GCONFIG_SetConfigPacket(GCONFIG_CFG_PKT* pCfgPkt)
{
    ...
    gconfig_ConfigData.WifiConfig.WifiProtectFrame = pCfgPkt->WifiConfig.WifiProtectFrame;
    ...
    gconfig_ConfigData.UpgradeCfg.TftpServerIpForBldr = pCfgPkt->TftpServerIpForBldr;
    memcpy((char *)gconfig_ConfigData.UpgradeCfg.FileNameBldr, (char *)pCfgPkt->FileNameBldr, 63);
    gconfig_ConfigData.UpgradeCfg.FileNameBldr[63] = 0;
    gconfig_ConfigData.UpgradeCfg.FileNameLenBldr = strlen(gconfig_ConfigData.UpgradeCfg.FileNameBldr);

    gconfig_ConfigData.UpgradeCfg.TftpServerIpForWcpu = pCfgPkt->TftpServerIpForWcpu;
    memcpy((char *)gconfig_ConfigData.UpgradeCfg.FileNameWcpu, (char *)pCfgPkt->FileNameWcpu, 63);
    gconfig_ConfigData.UpgradeCfg.FileNameWcpu[63] = 0;
    gconfig_ConfigData.UpgradeCfg.FileNameLenWcpu = strlen(gconfig_ConfigData.UpgradeCfg.FileNameWcpu);
...
void GCONFIG_GetConfigPacket(GCONFIG_CFG_PKT* pCfgPkt)
{
    ...
    pCfgPkt->WifiConfig.WifiProtectFrame = gconfig_ConfigData.WifiConfig.WifiProtectFrame;
#endif
    ...
    pCfgPkt->TftpServerIpForBldr = gconfig_ConfigData.UpgradeCfg.TftpServerIpForBldr;
    memcpy((char *)&pCfgPkt->FileNameBldr[0], (char *)&gconfig_ConfigData.UpgradeCfg.FileNameBldr[0], 64);

    pCfgPkt->TftpServerIpForWcpu = gconfig_ConfigData.UpgradeCfg.TftpServerIpForWcpu;
    memcpy((char *)&pCfgPkt->FileNameWcpu[0], (char *)&gconfig_ConfigData.UpgradeCfg.FileNameWcpu[0], 64);
...
void GCONFIG_ReadDefaultConfigData(void)
{
    ...
    gconfig_ConfigData.WifiConfig.WifiProtectFrame = 0;
#endif
	...
	gconfig_ConfigData.Gateway = 0xc0a80203; 		// 192.168.2.1						//zwh.20120209
    ...
    gconfig_ConfigData.DhcpSrv.DefaultGateway = 0xc0a80203;  /* 0.0.0.0  */			//zwh.20120209
 ...
 * ----------------------------------------------------------------------------
 */
void GCONFIG_EnableFirmwareUpgrade(U8_T type)
{
	FirmwareUpgradeFlag[0] = 'a';
	FirmwareUpgradeFlag[1] = 'x';
	FirmwareUpgradeFlag[2] = '_';

    if (type == GCONFIG_FTYPE_BLDR)
	    FirmwareUpgradeFlag[3] = 'b';
    else if (type == GCONFIG_FTYPE_WCPU)
	    FirmwareUpgradeFlag[3] = 'w';
    else
...
void GCONFIG_SetServerStaticIP(U32_T ip)
{
	...
	GCONFIG_SetGateway(ip);	//zwh.20120209
...
void GCONFIG_SetGateway(U32_T gateway)
{
	...
	gconfig_ConfigData.DhcpSrv.DefaultGateway = gateway;	   //zwh.20120209
 ...
 * ----------------------------------------------------------------------------
 */
void GCONFIG_SetTftpServerIp(U32_T ip, U8_T type)
{
    if (type == GCONFIG_FTYPE_BLDR)
	    gconfig_ConfigData.UpgradeCfg.TftpServerIpForBldr = ip;
    else if (type == GCONFIG_FTYPE_MCPU)
	gconfig_ConfigData.UpgradeCfg.TftpServerIpForMcpu = ip;
    else if (type == GCONFIG_FTYPE_WCPU)
	    gconfig_ConfigData.UpgradeCfg.TftpServerIpForWcpu = ip;
 ...
 * ----------------------------------------------------------------------------
 */
U8_T GCONFIG_SetFilename(U8_T *filename, U8_T len, U8_T type)
{
    ...
    if (type == GCONFIG_FTYPE_BLDR)
    {
	    memcpy(gconfig_ConfigData.UpgradeCfg.FileNameBldr, filename, len); 
	    gconfig_ConfigData.UpgradeCfg.FileNameBldr[len] = '\0';
        gconfig_ConfigData.UpgradeCfg.FileNameLenBldr = len;
    }
    else if (type == GCONFIG_FTYPE_MCPU)
    {
    ...
    {
	    memcpy(gconfig_ConfigData.UpgradeCfg.FileNameWcpu, filename, len); 
	    gconfig_ConfigData.UpgradeCfg.FileNameWcpu[len] = '\0';
        gconfig_ConfigData.UpgradeCfg.FileNameLenWcpu = len;
    }
    else
        return 0;
 ...
 * ----------------------------------------------------------------------------
 */
U32_T GCONFIG_GetTftpServerIp(U8_T type)
{
    if (type > GCONFIG_FTYPE_WCPU)
        return 0;

    if (type == GCONFIG_FTYPE_BLDR)
	    return gconfig_ConfigData.UpgradeCfg.TftpServerIpForBldr;
    else if (type == GCONFIG_FTYPE_MCPU)
	return gconfig_ConfigData.UpgradeCfg.TftpServerIpForMcpu; 

    return gconfig_ConfigData.UpgradeCfg.TftpServerIpForWcpu;
 ...
 * ----------------------------------------------------------------------------
 */
U8_T GCONFIG_GetFilename(U8_T *filename, U8_T len, U8_T type)
{
    if (!filename || len < GCONFIG_MAX_FILENAME_LEN || type > GCONFIG_FTYPE_WCPU)
        return 0;

    if (type == GCONFIG_FTYPE_BLDR)
    {
	    memcpy(filename, gconfig_ConfigData.UpgradeCfg.FileNameBldr, GCONFIG_MAX_FILENAME_LEN);
        len = gconfig_ConfigData.UpgradeCfg.FileNameLenBldr;
    }
    else if (type == GCONFIG_FTYPE_MCPU)
	{
        memcpy(filename, gconfig_ConfigData.UpgradeCfg.FileNameMcpu, GCONFIG_MAX_FILENAME_LEN);
        len = gconfig_ConfigData.UpgradeCfg.FileNameLenMcpu;
    }
    else if (type == GCONFIG_FTYPE_WCPU)
    {
	    memcpy(filename, gconfig_ConfigData.UpgradeCfg.FileNameWcpu, GCONFIG_MAX_FILENAME_LEN); 
        len = gconfig_ConfigData.UpgradeCfg.FileNameLenWcpu;
    }
...
/*
 * ----------------------------------------------------------------------------
 * Function Name: GCONFIG_GetProtectionFrameType
 * Purpose: 
 * Params:
 * Returns:
 * Note:
 * ----------------------------------------------------------------------------
 */
U8_T GCONFIG_GetProtectionFrameType(void)
{
	return gconfig_ConfigData.WifiConfig.WifiProtectFrame;
} /* End of GCONFIG_GetProtectionFrameType() */

/*
 * ----------------------------------------------------------------------------
 * Function Name: GCONFIG_SetProtectionFrameType
 * Purpose: 
 * Params:
 * Returns:
 * Note:
 * ----------------------------------------------------------------------------
 */
void GCONFIG_SetProtectionFrameType(U8_T value)
{
    if (value > 1)
        return;

	gconfig_ConfigData.WifiConfig.WifiProtectFrame = value;
		
} /* End of GCONFIG_SetProtectionFrameType() */
#endif
...
static void	gconfig_DisplayConfigData(GCONFIG_CFG_DATA *pConfigData)
{
	...
	if (GS2W_GetTaskState() == GS2W_STATE_IDLE)
	{
		...
		printd ("%04x   ", pConfigData->Network);		
		...
		printd("  Baseband: %s mode ", (i==1) ? "802.11b+g" : ((i==2) ? "802.11a" : "unsupport"));/* index:1/2 = 802.11b+g/802.11a */
		printd("\r\n");
		
		// Network mode //		
		i = pConfigData->WifiConfig.WifiNetworkMode;		
		printd("  Network: %s mode ", (i ? "Ad-hoc" : "Infrastructure"));/* index:1/0 = Ad-hoc/Infra */
		printd("\r\n");
		...
		// 64/128 bits Key Index 1 ~ 4 //
		for (val = 0; val < 4; val ++)
		{
			printd("  WEP64 bits Key index %u: 0x", val);
			for (i = 0; i < 5; i ++)
		...
		for (val = 0; val < 4; val ++)
		{
			printd("  WEP128 bits Key index %u: 0x", val);
			for (i = 0; i < 13; i ++)
		...
		i = pConfigData->WifiConfig.WifiProtectFrame;
		printd("  Protection Frame Type: ");
		printd("%s", (i ? "Self-CTS" : "RTS"));		
#endif			
