...
U8_T GUDPBC_NewConn(U32_T XDATA* pip, U16_T remotePort, U8_T socket)
{
	...
	printd("GUDPBC_NewConn():: socket=%bu, remoteport=%u, pip=%lu\r\n", socket, remotePort, *pip);
...
{
    printd("GUDPBC_Event():: id=%bu,event=%bu\r\n", id, event);
...
void GUDPBC_Receive(U8_T XDATA* pData, U16_T length, U8_T id)
{
    ...
    printd("GUDPBC_Receive():: id=%bu, length=%bu\r\n", id, length);
...
{	
    ...
    printd("gudpbc_HandleSearchReq()::, id=%bu, size=%u\r\n", id, sizeof(gudpuc_ConfigRxPkt));
...
void gudpbc_HandleSetReq(U8_T XDATA* pData, U16_T length, U8_T id)
{
    ...
    printd("gudpbc_HandleSetReq()::, id=%bu, length=%u\r\n", id, length);
...
void gudpbc_HandleUpgradeReq(U8_T XDATA* pData, U16_T length, U8_T id)
{	
    printd("gudpbc_HandleUpgradeReq()::, id=%bu, length=%u\r\n", id, length);
