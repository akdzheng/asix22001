...
void DNSC_Timer()
	...
	if (TimerElapse > SWTIMER_COUNT_SECOND)
	{
		...
		for (i = 0; i< MAX_DNSC_RECORD_CNT; i ++)	//Add for recorder expire function.
		{
...
DNSC_STATE DNSC_Start(U8_T *pHostName)
{
	...
	for (Index = 0 ; Index < MAX_DNSC_RECORD_CNT ; Index ++)//Set the host name to query
	{
		if (dnscInfo.Table[Index].Result == DNSC_QUERY_FREE)
			break;
	}

	if (Index != MAX_DNSC_RECORD_CNT)
	{
	...
	i = 0;
	for (Index = 0; Index < MAX_DNSC_RECORD_CNT; Index ++)
	{
...
DNSC_STATE DNSC_Query(U8_T *pHostName, U32_T *pHostIP)
{
	...
	for (Index = 0 ; Index < MAX_DNSC_RECORD_CNT ; Index ++)//Find host IP in record table.
	{
	...
	for (Index = 0 ; Index < MAX_DNSC_RECORD_CNT ; Index ++)//Check this host IP query.
	{
...
void DNSC_Task()
{
	...
	case DNSC_TASK_RESPONSE://Receive response packet OK
		...
		else
		{
			...
			dnscInfo.Table[Index].TimerToLive = MAX_DNSC_RECORD_EXPIRE_TIME;	//Add for recorder expire function.
			dnscInfo.TaskState = DNSC_TASK_START;
		}
		break;
