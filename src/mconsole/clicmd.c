...
static S16_T clicmd_SetProtectFrame(CONSOLE_Inst *pInst);
static S16_T clicmd_SetProtectFrameHelp(CONSOLE_Inst *pInst);
...
static S16_T clicmd_Show(CONSOLE_Inst *pInst);
static S16_T clicmd_ShowHelp(CONSOLE_Inst *pInst);
...
static S16_T clicmd_SetServerPort(CONSOLE_Inst *pInst)
{
    ...
    if (temp32 < 1024 || temp32 > 65535)
        return -1;
    
    GCONFIG_SetServerDataPktListenPort((U16_T)temp32);
...
static S16_T clicmd_SetEtherTimer(CONSOLE_Inst *pInst)
{
    ...
    if (temp32 < 10 || temp32 > 65535)
        return -1;

    GCONFIG_SetEthernetTxTimer((U16_T)temp32);
...
static S16_T clicmd_SetRemotePort(CONSOLE_Inst *pInst)
{
    ...
    GCONFIG_SetClientDestPort((U16_T)temp32);
...
static S16_T clicmd_SetTftpServerIp(CONSOLE_Inst *pInst)
{
    ...
    if (argc == 0) /* current setting */
    {
        addr = GCONFIG_GetTftpServerIp(GCONFIG_FTYPE_BLDR);
        CONSOLE_PutMessage(pInst, "TFTP Server IP address for Bldr: %bu.%bu.%bu.%bu\r\n", (U8_T)((addr>>24)&0x000000FF),
                           (U8_T)((addr>>16)&0x000000FF), (U8_T)((addr>>8)&0x000000FF), (U8_T)(addr&0x000000FF));
        addr = GCONFIG_GetTftpServerIp(GCONFIG_FTYPE_MCPU);
        CONSOLE_PutMessage(pInst, "TFTP Server IP address for MCPU: %bu.%bu.%bu.%bu\r\n", (U8_T)((addr>>24)&0x000000FF),
                           (U8_T)((addr>>16)&0x000000FF), (U8_T)((addr>>8)&0x000000FF), (U8_T)(addr&0x000000FF));
        addr = GCONFIG_GetTftpServerIp(GCONFIG_FTYPE_WCPU);
        CONSOLE_PutMessage(pInst, "TFTP Server IP address for WCPU: %bu.%bu.%bu.%bu\r\n", (U8_T)((addr>>24)&0x000000FF),
                           (U8_T)((addr>>16)&0x000000FF), (U8_T)((addr>>8)&0x000000FF), (U8_T)(addr&0x000000FF));
        return 1;
    }
    else if (argc != 2)
    ...
    {
        if (clicmd_ValidIp(addr))
            GCONFIG_SetTftpServerIp(addr, value);
        else
            return -1;
    }
    else
...
static S16_T clicmd_SetTftpServerIpHelp(CONSOLE_Inst *pInst)
{
    CONSOLE_PutMessage(pInst, "Usage: tftpsrv <type> <ip addr>\r\n");
    CONSOLE_PutMessage(pInst, "       <type>: 0: Bldr    1: MCPU    2: WCPU\r\n");
...
static S16_T clicmd_SetFilename(CONSOLE_Inst *pInst)
{
    ...
    if (argc == 0) /* current setting */
    {
		GCONFIG_GetFilename((U8_T*)StrBuf, 64, GCONFIG_FTYPE_BLDR);
 		CONSOLE_PutMessage(pInst, "Bldr Filename: %s\r\n", StrBuf);
		GCONFIG_GetFilename((U8_T*)StrBuf, 64, GCONFIG_FTYPE_MCPU);
 		CONSOLE_PutMessage(pInst, "MCPU Filename: %s\r\n", StrBuf);
		GCONFIG_GetFilename((U8_T*)StrBuf, 64, GCONFIG_FTYPE_WCPU);
 		CONSOLE_PutMessage(pInst, "WCPU Filename: %s\r\n", StrBuf);
        return 1;
    }
    else if (argc != 2)
        return -1;
    ...
    GCONFIG_SetFilename(argv[1], strlen(argv[1]), value);
    return 1;
...
static S16_T clicmd_SetFilenameHelp(CONSOLE_Inst *pInst)
{
    CONSOLE_PutMessage(pInst, "Usage: filename <type> <file name>\r\n");
    CONSOLE_PutMessage(pInst, "       <type>: 0: Bldr    1: MCPU    2: WCPU\r\n");
    return 1;
...
static S16_T clicmd_UpdateFirmware(CONSOLE_Inst *pInst)
{
    ...
    if (value == GCONFIG_FTYPE_BLDR)
        CONSOLE_PutMessage(pInst, "Bldr\r\n");
    else if (value == GCONFIG_FTYPE_MCPU)
        CONSOLE_PutMessage(pInst, "MCPU\r\n");
    else
        CONSOLE_PutMessage(pInst, "WCPU\r\n");
         
    GCONFIG_EnableFirmwareUpgrade(value);
        GCONFIG_WriteConfigData();
        MCPU_SoftReboot();
...
static S16_T clicmd_UpdateFirmwareHelp(CONSOLE_Inst *pInst)
{
    CONSOLE_PutMessage(pInst, "Usage: dlfirmware <type>\r\n");
    CONSOLE_PutMessage(pInst, "       <type>: 0: Bldr    1: MCPU    2: WCPU\r\n");
    return 1;
...
static S16_T clicmd_Ping(CONSOLE_Inst *pInst)
{
	...
	if (pInst->State == CLI_STATE_COMMAND)	
	{
 		if (argc != 1)
        	return -1;
    	if (clicmd_GetIp2Ulong(argv[0], strlen(argv[0]), &(pPing->TargetIP)) < 0 || !clicmd_ValidIp(pPing->TargetIP))
		{
...
static S16_T clicmd_SetSSIDHelp(CONSOLE_Inst *pInst)
{
    CONSOLE_PutMessage(pInst, "Usage: setssid <Maximum 31 characters>\r\n");
    return 1;
...
static S16_T clicmd_SetNetworkType(CONSOLE_Inst *pInst)
{
    ...
    if (value < 2)
    {
		...
		IO_NetworkModeRW(1, value ? 2:3);/* 2/3: Ad-hoc/infra */
	}
	else
		return -1;
...
static S16_T clicmd_SetPreamble(CONSOLE_Inst *pInst)
{
	...
	IO_PreambleRW(1, value ? 0x20:0); /* 0/0x20: long/auto */
	return 1;
...
static S16_T clicmd_SetDataRate(CONSOLE_Inst *pInst)
{
	...
	else if (argc != 1 || strlen(argv[0]) > 2) /* Write user's data rate */
        return -1;
	...
	if (IO_TxDataRateRW(1, IO_WiFiSupportTxRate[Value]))
	{
		return -1;
	}
	...
	else if (pInst->State == CLI_STATE_COMMAND_WAIT)
	{
		if  (pInst->Cmd.ReplyLen && (pInst->Cmd.Buf[0] + pInst->Cmd.Buf[1]) == 0xFF)
		{
			...
			case IO_SELF_CTS_RESP:
				...
				CONSOLE_PutMessage(pInst, "* Protection Frame Type|%bu: ",pInst->Cmd.Buf[4]);
				if (pInst->Cmd.Buf[4] == 0)
					CONSOLE_PutMessage(pInst, "RTS\r\n");
				else if (pInst->Cmd.Buf[4] == 1)
					CONSOLE_PutMessage(pInst, "Self-CTS\r\n");
				else
					CONSOLE_PutMessage(pInst, "Unsupport\r\n");
...
/*
 * ----------------------------------------------------------------------------
 * Function Name: clicmd_SetProtectFrame()
 * Purpose:
 * Params:
 * Returns:
 * Note:
...
static S16_T clicmd_SetProtectFrame(CONSOLE_Inst *pInst)
{
	...
	if (argc == 0) /* Read current beacon interval */
	{
        StrLen = GCONFIG_GetProtectionFrameType();
        CONSOLE_PutMessage(pInst, "Protection frame type: ");
        if (StrLen)
            CONSOLE_PutMessage(pInst, "Self-CTS\r\n");
        else
            CONSOLE_PutMessage(pInst, "RTS\r\n");
	...
	GCONFIG_SetProtectionFrameType(StrLen);
	IO_SelfCtsRW(1, StrLen);
	return 1;

} /* End of clicmd_SetProtectFrame() */

/*
 * ----------------------------------------------------------------------------
 * Function Name: clicmd_SetProtectFrameHelp()
 * Purpose:
 * Params:
 * Returns:
 * Note:
...
static S16_T clicmd_SetProtectFrameHelp(CONSOLE_Inst *pInst)
{
    CONSOLE_PutMessage(pInst, "Usage: setpft <Protection frame type>\r\n");
    CONSOLE_PutMessage(pInst, "       <Protection frame type>: 0: RTS    1: Self-CTS\r\n");

    return 1;
} /* End of clicmd_SetProtectFrameHelp() */
#endif
...
static S16_T clicmd_SetDhcpServer(CONSOLE_Inst *pInst)
{
    ...
    if (argc == 0) /* current setting */
    {
        ...
        CONSOLE_PutMessage(pInst, "Gateway: %bu.%bu.%bu.%bu\r\n", (U8_T)((addr>>24)&0x000000FF),
                           (U8_T)((addr>>16)&0x000000FF), (U8_T)((addr>>8)&0x000000FF), (U8_T)(addr&0x000000FF));
        CONSOLE_PutMessage(pInst, "Lease Time: %u\r\n", GCONFIG_GetDhcpSrvLeaseTime());
    ...
    if ((pAddr[0] >= pAddr[1]) || (pAddr[0]&pAddr[2]) != (pAddr[1]&pAddr[2]))
        return -1;
    ...
    GCONFIG_SetDhcpSrvLeaseTime(argc);
...
static S16_T clicmd_SetDhcpServerHelp(CONSOLE_Inst *pInst)
{
    CONSOLE_PutMessage(pInst, "Usage: dhcpsrv <start addr> <end addr> <netmask> <gateway> <lease>\r\n");
    return 1;
...
//zwh.add bellow 20120302
/*
 * ----------------------------------------------------------------------------
 * Function Name: clicmd_Show()
 * Purpose:
 * Params:
 * Returns:
 * Note:
...
static S16_T clicmd_Show(CONSOLE_Inst *pInst)
{
	...
	if ((argc == 1) && (strcmp(argv[0], "counter")==0))
	{
		CONSOLE_PutMessage(pInst, "connect try     = %lu\r\n", COUNTER_GET_CONNECT_TRY);
		CONSOLE_PutMessage(pInst, "connect failed  = %lu\r\n", COUNTER_GET_CONNECT_FAILED);
		CONSOLE_PutMessage(pInst, "connect success = %lu\r\n", COUNTER_GET_CONNECT_SUCCESS);
		CONSOLE_PutMessage(pInst, "connect closed  = %lu\r\n", COUNTER_GET_CONNECT_CLOSED);
		return 1;
	}
	return -1;
} /* End of clicmd_EnableDhcpServer() */
...
static S16_T clicmd_ShowHelp(CONSOLE_Inst *pInst)
{
    CONSOLE_PutMessage(pInst, "Usage: show <item> \r\n");
    CONSOLE_PutMessage(pInst, "       <item>: counter \r\n");

    return 1;

} /* End of clicmd_EnableDhcpServerHelp() */			  //zwh.add above 20120302
